...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # ✅ this link will be used by frontend
            reset_url = f"http://localhost:5173/reset-password/{uid}/{token}/"

            # instead of sending the email here, return the link to frontend
            return Response({
...
#         # ✅ If user is authenticated, ensure email matches their own
#         if request.user.is_authenticated:
#             if request.user.email != email:
#                 return Response(
#                     {"error": "You can only request a password reset for your own account."},
#                     status=403
#                 )
...
class UserPrivacyView(generics.RetrieveUpdateAPIView):
    ...
    def get_object(self):
        # Get or create the privacy settings for the current user
...
#         """
#         Delete the currently authenticated user's account after verifying password.
#         """
...
class DeleteAccountView(generics.GenericAPIView):
    ...
    def delete(self, request):
        ...
        # Case 1: User has a password → require password verification
        ...
        # Case 2: OAuth user → skip password verification
        # Delete account
        user.delete()
